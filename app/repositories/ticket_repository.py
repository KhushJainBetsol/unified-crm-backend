...
class TicketRepository:
    ...
    async def get_all(
        ...
        if status:
            query = query.join(TicketStatus, Ticket.status_id == TicketStatus.id).where(
                TicketStatus.status_name == status.lower()
            )
            count_query = count_query.join(TicketStatus, Ticket.status_id == TicketStatus.id).where(
                TicketStatus.status_name == status.lower()
            )

        if priority:
            query = query.join(TicketPriority, Ticket.priority_id == TicketPriority.id).where(
                TicketPriority.priority_name == priority.lower()
            )
            count_query = count_query.join(TicketPriority, Ticket.priority_id == TicketPriority.id).where(
                TicketPriority.priority_name == priority.lower()
            )
