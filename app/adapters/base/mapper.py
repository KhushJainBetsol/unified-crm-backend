...
class SchemaMapper:
    ...
    def to_customer(self, raw: Dict[str, Any]) -> UnifiedCustomer:
        """Map a raw CRM user payload to a UnifiedCustomer."""
        # Use 'customer' mapping if defined in config, otherwise fallback to 'agent' mapping safely
        m = getattr(self._mappings, "customer", getattr(self._mappings, "agent", {}))
    ...
    def map_agents(self, raw_list: List[Dict[str, Any]]) -> List[UnifiedAgent]:
        ...
        return agents
    def map_customers(self, raw_list: List[Dict[str, Any]]) -> List[UnifiedCustomer]:
        """Bulk-map a list of raw customer dicts."""
        customers = []
        for raw in raw_list:
            try:
                customers.append(self.to_customer(raw))
            except Exception:
                logger.exception(
                    "SchemaMapper failed to map customer payload (id=%s). Skipping.",
                    raw.get("id"),
                )
        return customers
    # ------------------------------------------------------------------
